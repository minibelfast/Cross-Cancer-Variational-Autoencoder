...
cell_types = ['Macrophages', 'Fibroblasts']
...
for ct in cell_types:
    ...
    if adata_sct.shape[0] > 20000:
        sc.pp.subsample(adata_sct, n_obs=20000, random_state=42)
        print(f"Subsampled to {adata_sct.shape[0]} cells for scTour training.")
        
    print("Training scTour model...")
    try:
        tn = sct.train.Trainer(adata_sct, loss_mode='mse', alpha_recon_lec=0.5, alpha_recon_lode=0.5)
        tn.train(n_epochs=20)
    except TypeError:
        tn = sct.train.Trainer(adata_sct, loss_mode='mse', alpha_recon_lec=0.5, alpha_recon_lode=0.5)
        tn.train()
