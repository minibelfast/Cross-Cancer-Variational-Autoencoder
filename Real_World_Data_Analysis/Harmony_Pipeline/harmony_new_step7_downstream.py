...
for ct in cell_types:
    ...
    plt.title(f"{ct} Subtype Proportions by Cancer Type")
...
# We will define some pan-cancer pathways
pathways = {
...
for ct in cell_types:
    in_file = f"{out_dir}/10_{ct.replace(' ', '')}_adata_sctour_harmony.h5ad"
    if not os.path.exists(in_file):
        continue
    ...
    # Dotplot of pathways across cell types
    ...
    # Pseudotime correlation with genes to find common drivers
    if 'scTour_pseudotime' in adata.obs.columns:
        print(f"Finding genes correlated with pseudotime in {ct}...")
        # Subset to highly variable genes to speed up
        sc.pp.highly_variable_genes(adata, n_top_genes=1000)
        adata_hv = adata[:, adata.var['highly_variable']].copy()
        
        # Calculate correlation
        pt = adata_hv.obs['scTour_pseudotime'].values
        ...
        # Plot top 5 positive and negative correlated genes
