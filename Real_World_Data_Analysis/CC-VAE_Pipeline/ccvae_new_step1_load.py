...
print("=== CC-VAE Integration ===", flush=True)
# Initialize and train CC-VAE
# Note: CrossCancerVAE automatically calls scvi.model.SCVI.setup_anndata in its __init__
model = CrossCancerVAE(adata_concat, batch_key="Batch")
print("Training CC-VAE (this may take a while)...", flush=True)
model.train(max_epochs=200, early_stopping=True)

print("Extracting latent representation...", flush=True)
adata_concat.obsm["X_ccvae"] = model.get_latent_representation()

print("Clustering and UMAP...", flush=True)
sc.pp.neighbors(adata_concat, use_rep='X_ccvae', n_neighbors=30)
sc.tl.umap(adata_concat)
sc.tl.leiden(adata_concat, resolution=0.8, key_added='leiden_ccvae')
...
sc.pl.umap(adata_concat, color='leiden_ccvae', show=False, ax=axs[2])
plt.tight_layout()
plt.savefig(f"{out_dir}/01_All_Cells_UMAP_CCVAE.pdf", bbox_inches='tight')

print("Calculating Markers...", flush=True)
sc.tl.rank_genes_groups(adata_concat, groupby='leiden_ccvae', method='wilcoxon')
markers_df = pd.DataFrame(adata_concat.uns['rank_genes_groups']['names'])
markers_df.to_csv(f"{out_dir}/02_All_Cells_Cluster_Markers_CCVAE.csv", index=False)

print("Saving AnnData...", flush=True)
adata_concat.write(f"{out_dir}/integrated_adata_ccvae.h5ad")
print("=== CC-VAE Step 1 Complete ===", flush=True)
