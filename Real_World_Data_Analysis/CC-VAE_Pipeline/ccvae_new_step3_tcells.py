...
ct = 'T cells'
print(f"\n{'='*40}")
print(f"Processing {ct} for CC-VAE")
print(f"{'='*40}")

in_file = f"{out_dir}/05_{ct.replace(' ', '')}_adata_ccvae.h5ad"
...
# For CC-VAE subsets, we still use CC-VAE or Harmony? 
# Usually, people re-integrate subsets. We can re-run CC-VAE or just use Harmony for speed on subsets.
# The user's notebook test1.ipynb usually re-runs Harmony or CC-VAE. 
# Let's use CC-VAE since this is the CC-VAE pipeline.
...
print("=== CC-VAE Sub-Integration ===", flush=True)
model = CrossCancerVAE(adata, batch_key="Batch")
print("Training CC-VAE on subset...", flush=True)
model.train(max_epochs=200, early_stopping=True)

adata.obsm["X_ccvae_sub"] = model.get_latent_representation()

print("Clustering...")
sc.pp.neighbors(adata, use_rep='X_ccvae_sub', n_neighbors=20)
sc.tl.umap(adata)
...
adata.write(f"{out_dir}/08_{ct.replace(' ', '')}_adata_fine_ccvae.h5ad")

print(f"=== Step 3 {ct} Complete ===")
