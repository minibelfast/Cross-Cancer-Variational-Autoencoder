...
for ct in cell_types:
    ...
    # Load the fine-annotated subclusters
    ...
    # Preprocessing for scTour as in harmony_step4_sctour.py
    if 'highly_variable' in adata.var:
        adata_sct = adata[:, adata.var['highly_variable']].copy()
    else:
        # Assuming counts are in adata.X or layers['counts']
        if 'counts' in adata.layers:
            adata.X = adata.layers['counts'].copy()
        sc.pp.highly_variable_genes(adata, n_top_genes=2000, flavor='cell_ranger', subset=False)
        adata_sct = adata[:, adata.var['highly_variable']].copy()
    ...
    # Using loss_mode='mse' and nepoch=20 as in the correct script
    ...
    if 'X_T' not in adata_sct.obsm:
        # get_latentsp might save directly to obsm in older versions
        if 'X_T' not in adata_sct.obsm:
    ...
    sc.pl.umap(adata_sct, color=['CellType_Fine', 'ptime'], save=f"_{ct.replace(' ', '')}_scTour_ptime.pdf", show=False)
    ...
    adata.write(f"{out_dir}/10_{ct.replace(' ', '')}_adata_sctour_ccvae_fixed.h5ad")

print("=== scTour Analysis Complete ===")
