...
for cancer, path in datasets.items():
    ...
    for mat_file in matrix_files:
        ...
        try:
            ...
            var_names = set([str(x).upper() for x in var_names])
            
            if current_common is None:
                current_common = var_names
            else:
                current_common = current_common.intersection(var_names)
            
            print(f"{os.path.basename(prefix)}: {len(var_names)} genes, Common so far: {len(current_common)}")
